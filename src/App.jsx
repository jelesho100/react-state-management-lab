...
const App = () => {
  ...
  const handleAddFighter = (fighter) => {
    ...
    setTeam((myTeam) => [...myTeam, fighter]);

    setZombieFighters((prevFighters) =>
      prevFighters.filter((fighterObj) => fighterObj.id !== fighter.id)
    );

    setMoney((myMoney) => myMoney - fighter.price);
  }

  const handleRemoveFighter = (fighter) => {
  setTeam((myTeam) =>
    myTeam.filter((fighterObj) => fighterObj.id !== fighter.id)
  );
  ...
  if (team.length === 0) {
    teamDisplay = <p>Pick some Fighters!</p>;
  } else {
    teamDisplay = (
      <ul>
        {team.map((fighter) => (
